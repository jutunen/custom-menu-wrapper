...
export class Custommenu extends HTMLElement {
  ...
  _addEntry (entry, index) {
    ...
    let menu = entry.querySelector('custom-menu-wrapper')
    if (menu) {
  ...
  _init (checkRecursion) {
    ...
    if (entry.done === true) {
      let menuContent = this.querySelector('custom-menu-wrapper > *')
...
customElements.define('custom-menu-wrapper', Custommenu)
